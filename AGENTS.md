...
- Admin-only routes use `meta: { requiresAdmin: true }` — guard checks `user_metadata.role` for both `'admin'` and `'superadmin'`
- 404 catch-all redirect → `{ name: 'Dashboard' }`
- `.env` requires 4 vars: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_N8N_WEBHOOK_URL`, `VITE_N8N_WEBHOOK_SECRET`. Also needs `NGROK_DOMAIN` for Docker ngrok service
- Docker and frontend **share `.env`** — docker-compose.yml uses `env_file: .env`
...
- `.env.n8n` from `.env.n8n.example`. CORS configured for `http://localhost:5173`
