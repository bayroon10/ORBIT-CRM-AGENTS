...
const qualifyLead = async () => {
  ...
  // URL de la Edge Function que actúa como proxy
  const EDGE_FUNCTION_URL = 'https://kgrfhfwtcanthrymmvkb.supabase.co/functions/v1/quick-processor'; // <--- USAR TU URL AQUI

  // No necesitamos validar VITE_N8N_WEBHOOK_URL/SECRET aquí, la Edge Function maneja eso.
  // Ni tampoco el fallback de /webhook/ a /webhook-test/, la Edge Function solo llama a la URL configurada.
  ...
  try {
    const response = await fetch(EDGE_FUNCTION_URL, {
      ...
      headers: {
        ...
        // El secreto ya no se envía desde el frontend, lo inyecta la Edge Function
      },
