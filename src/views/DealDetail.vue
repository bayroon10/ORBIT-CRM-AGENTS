    ...
    <template v-else-if="deal">
      <div class="relative overflow-hidden rounded-xl bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 px-8 py-10 mb-8 shadow-2xl border border-white/10 shrink-0">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 class="text-3xl font-bold text-slate-50">{{ deal.title }}</h1>
            <p class="text-slate-400 mt-1">{{ (deal.stage ? deal.stage.charAt(0).toUpperCase() + deal.stage.slice(1) : '') + ' · ' + (deal.leads?.full_name || deal.companies?.name || 'Sin contacto') }}</p>
          </div>
      ...
      <div class="flex-1 overflow-y-auto pb-8 mt-6 px-6">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-1">
            <BaseCard class="mb-6">
              <h2 class="font-semibold text-slate-50 mb-4 text-lg">Detalles</h2>
              <div class="space-y-0">
                
                <div class="flex items-center justify-between py-3 border-b border-white/10">
                  <div class="text-xs text-slate-500 uppercase tracking-wide">Valor</div>
                  <div class="text-sm text-success font-semibold">{{ formatCurrency(deal.value) }}</div>
                </div>

                <div class="flex items-center justify-between py-3 border-b border-white/10">
                  <div class="text-xs text-slate-500 uppercase tracking-wide">Probabilidad</div>
                  <div class="text-sm text-slate-50 font-medium">{{ deal.probability != null ? deal.probability + '%' : '—' }}</div>
                </div>

                <div class="flex items-center justify-between py-3 border-b border-white/10">
                  <div class="text-xs text-slate-500 uppercase tracking-wide">Cierre Esperado</div>
                  <div class="text-sm font-medium" :class="isPastDue(deal.expected_close) ? 'text-danger' : 'text-slate-50'">
                    {{ formatDate(deal.expected_close) }}
                  </div>
                </div>

                <div class="flex items-center justify-between py-3 border-b border-white/10">
                  <div class="text-xs text-slate-500 uppercase tracking-wide">Contacto</div>
                  <div class="text-sm text-slate-50 font-medium">{{ deal.leads?.full_name || '—' }}</div>
                </div>

                <div class="flex items-center justify-between py-3 border-b border-white/10">
                  <div class="text-xs text-slate-500 uppercase tracking-wide">Empresa</div>
                  <div class="text-sm text-slate-50 font-medium">{{ deal.companies?.name || '—' }}</div>
                </div>

                <div class="flex items-center justify-between py-3 border-b border-white/10">
                  <div class="text-xs text-slate-500 uppercase tracking-wide">Creado</div>
                  <div class="text-sm text-slate-50 font-medium">{{ formatDate(deal.created_at) }}</div>
                </div>

                <div class="flex items-center justify-between py-3">
                  <div class="text-xs text-slate-500 uppercase tracking-wide">Actualizado</div>
                  <div class="text-sm text-slate-50 font-medium">{{ formatDate(deal.updated_at) }}</div>
                </div>
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <BaseCard :padded="false" class="bg-slate-800/50 border-white/10 overflow-hidden relative">
              ...
              <div class="p-6">
                <div class="flex justify-between items-start mb-4 relative z-10">
                  <div class="flex items-center gap-2">
                    ...
                    <h2 class="font-bold text-slate-50 text-lg tracking-wide">Deal Health</h2>
                  </div>
                  <BaseBadge v-if="deal.ai_risk_score != null" :variant="getRiskVariant(deal.ai_risk_score)">
                ...
                <div class="relative z-10">
                  <template v-if="deal.ai_risk_score != null">
                    <p class="text-sm text-slate-50 leading-relaxed font-medium mb-3">
            ...
            <BaseCard class="mb-6">
              <div class="flex justify-between items-center mb-4">
                <h2 class="font-semibold text-slate-50 text-lg">Tareas</h2>
                <span class="bg-slate-800/50 border border-white/10 text-slate-400 px-2.5 py-0.5 rounded-full text-xs font-medium">{{ tasks.length }}</span>
              </div>
              
              <div v-if="tasks.length === 0" class="text-center py-4">
                <p class="text-sm text-slate-400">Sin tareas asociadas</p>
              </div>
              
              <div v-else class="space-y-0">
                <div v-for="task in tasks" :key="task.id" class="flex justify-between items-start gap-3 py-3 border-b border-white/10 last:border-0 hover:bg-slate-800/50 transition-colors -mx-6 px-6">
                  <div>
                    <div class="font-medium text-sm text-slate-50">{{ task.title }}</div>
                    <div class="text-xs mt-0.5" :class="(isPastDue(task.due_date) && task.status !== 'completada') ? 'text-danger' : 'text-slate-400'">
...
const fetchDeal = async () => {
  ...
  try {
    const { data, error: dealError } = await DealsService.getDealById(route.params.id)
    ...
    const [activitiesResponse, tasksResponse] = await Promise.all([
      DealsService.getDealActivities(route.params.id),
      DealsService.getDealTasks(route.params.id)
    ])
