  ...
  <div class="flex flex-col h-full">
    ...
    <div class="relative overflow-hidden rounded-xl bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 px-8 py-10 mb-8 border border-white/10 shadow-2xl shrink-0">
      ...
      <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-3xl font-bold text-slate-50 mb-2 tracking-tight">Empresas</h1>
          <p class="text-slate-400 text-sm">Gestión de empresas y clientes corporativos.</p>
        </div>
      ...
      <div v-else-if="filteredCompanies.length > 0" class="overflow-x-auto overflow-y-auto flex-1">
        <table class="w-full text-left text-sm">
          <thead class="bg-slate-950/50 backdrop-blur-md text-slate-400 font-semibold sticky top-0 z-10">
            <tr>
              <th class="px-6 py-4 border-b border-white/10 uppercase tracking-wider text-xs">Empresa</th>
              <th class="px-6 py-4 border-b border-white/10 uppercase tracking-wider text-xs hidden sm:table-cell">Industria</th>
              <th class="px-6 py-4 border-b border-white/10 uppercase tracking-wider text-xs hidden sm:table-cell">Sitio Web</th>
              <th class="px-6 py-4 border-b border-white/10 uppercase tracking-wider text-xs hidden md:table-cell">Fecha Creación</th>
            </tr>
          </thead>
            ...
            >
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-lg bg-slate-800/50 flex items-center justify-center overflow-hidden border border-white/10 text-primary-300 font-bold text-lg shrink-0">
                    {{ company.name.charAt(0).toUpperCase() }}
                  </div>
                  <div class="font-medium text-slate-50 group-hover:text-primary-300 transition-colors">{{ company.name }}</div>
                </div>
              </td>
    ...
    <OrbitModal v-model="showModal" title="Nueva Empresa">
      <form @submit.prevent="submitCompany" class="space-y-4">
        ...
        <div>
          <label class="block text-sm font-medium text-slate-50 mb-1">Nombre de la Empresa *</label>
          <input
            v-model="form.name"
            type="text"
            required
...
const submitCompany = async () => {
  ...
  try {
    const { error: insertError } = await CompaniesService.createCompany({
      name: form.name.trim(),
